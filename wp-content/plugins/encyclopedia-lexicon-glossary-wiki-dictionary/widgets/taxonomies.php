...
class Taxonomies_Widget extends WP_Widget {
  ...
  function __construct(){
    parent::__construct (
      ...
      sprintf(I18n::t('%s Taxonomies'), Post_Type_Labels::getEncyclopediaType()),
      Array('description' => I18n::t('A list of your taxonomy terms.'))
    );
  }
  ...
  function Form($options){
    ...
    <p>
      ...
      <label for="<?php echo $this->get_Field_Id('show_count') ?>"><?php printf(I18n::t('Display number of %s.'), Post_Type_Labels::getItemPluralName()) ?></label>
    </p>
    ...
    <p>
      ...
      <select id="<?php echo $this->get_Field_Id('orderby') ?>" name="<?php echo $this->get_Field_Name('orderby') ?>" class="widefat">
        ...
        <option value="count" <?php selected($options->orderby, 'count') ?>><?php printf(I18n::t('Number of %s'), Post_Type_Labels::getItemPluralName()) ?></option>
        <option value="id" <?php selected($options->orderby, 'id') ?>>ID</option>
        <option value="slug" <?php selected($options->orderby, 'slug') ?>><?php echo I18n::t('URL Slug') ?></option>
