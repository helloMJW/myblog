...
if (!function_exists('venicelite_socials_function')) {

	function venicelite_socials_function() {
		...
		foreach ( $socials as $name => $attrs) { 
			...
			if (venicelite_setting('venicelite_footer_'.$name.'_button')): 

				$i++;	
				$html.= '<a href="'.esc_url(venicelite_setting('venicelite_footer_'.$name.'_button'), array( 'http', 'https', 'tel', 'skype', 'mailto' ) ).'" target="'.$attrs['target'].'" title="'.$name.'" class="social"> <i class="'.$attrs['icon'].'" ></i> </a> ';
			
			endif;
			
		}
		
		if ( venicelite_setting('venicelite_footer_rss_button') == "on" ): 
	...
	add_action( 'venicelite_socials', 'venicelite_socials_function', 10, 2 );
